...
public class Koneksi {
    ...
    */
    ...
    public static Connection getConnection() {
        ...
        String password = "12345678";
        try {
            File file = new File(new File("src/others").getAbsoluteFile(), "geulis.sql");
            String sqlScript = new String(Files.readAllBytes(file.toPath()));
            Connection connection = DriverManager.getConnection(jdbcURL, username, password);
            Statement statement = connection.createStatement();
            
            Statement stmt = connection.createStatement();
            ResultSet rst = stmt.executeQuery("SELECT SCHEMA_NAME FROM information_schema.schemata WHERE SCHEMA_NAME = 'geulis'");
            if(rst.next()) {
                return;
            }
